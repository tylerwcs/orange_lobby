...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const wb = buildLinksWorkbook((await listAttendees(ev.id)).map((a) => ({ name: a.name, email: a.email, category: a.category, table_no: fieldValue(a, "table_no"), link: attendeeLink(base, ev.slug, a.token) })));
