...
export function buildAttendanceWorkbook(attendees: Attendee[], checkpoints: Pick<Checkpoint, "id" | "name">[], checkins: Pick<Checkin, "checkpoint_id" | "attendee_id" | "scanned_at" | "scanned_by">[], scannerNames: Record<string, string>): ExcelJS.Workbook {
  ...
  ws.addRow(["Name", "Email", "Phone", "Company", "Category", "Table", "Source", ...extraKeys, ...checkpoints.flatMap((c) => [`${c.name} checked in`, `${c.name} time`, `${c.name} scanned by`])]);
  for (const a of attendees) {
    const row: (string | null)[] = [a.name, a.email, a.phone, a.company, a.category, a.table_no, a.source, ...extraKeys.map((k) => a.extra?.[k] ?? "")];
