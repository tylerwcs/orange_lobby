 ...
 *
 * The colour is decoration with a job: it groups sessions so a two-day programme can be
 * scanned. It carries no meaning of its own — there is no legend — so nothing reads it
 * back, nothing filters on it, and a session with no colour is exactly what it was before.
 */
...
/**
 * The class for the bar down the leading edge of a session, and for the dot beside its
 * title in the organiser's list. Both are the same fill, so they are one function.
 */
