...
export function NowCard({ next, href, today }: { next: { item: AgendaItem; status: "now" | "next" } | null; href: string; today: string }) {
  ...
  return (
    <Link href={`${href}?day=${item.day}`} className="flex items-center gap-3 rounded-[var(--radius-card)] border border-line bg-surface p-4 active:bg-canvas focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand">
