    ...
    <form action={addColumn} className="grid gap-4">
      {suggestions.length > 0 && (
        <div>
          ...
          <p className="mt-0.5 text-xs text-muted-foreground">Name a column after one of these and it arrives filled in.</p>
      ...
      <label className="block text-sm">
        ...
        <span className="mt-1 block text-xs text-muted-foreground">{HINTS[type]}</span>
      </label>
      ...
      {type === "select" && (
        <label className="block text-sm">
          ...
          <span className="mt-1 block text-xs text-muted-foreground">Separated by commas.</span>
        </label>
      )}
